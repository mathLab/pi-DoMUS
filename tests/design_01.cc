...
template <int dim, int spacedim>
class NFieldsProblem
{
public:
  NFieldsProblem(const Interface<dim,spacedim> &pb) :
    problem(pb),
...
int main ()
{
  ...
  NFieldsProblem<2,2> navier_stoks_driver(interface0);
  NFieldsProblem<2,2> stokes_driver(interface1);
