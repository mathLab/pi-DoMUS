...
public:
   ...
   *  - @p system_op represents the system matrix associated to the Newton's iterations;
   *  - @p prec_op represents the preconditioner;
   *  - @p prec_op_finer represents a finer preconditioner that can be used in the case the problem does not converge using @p prec_op .
   *
   * To clarify the difference between @p prec_op and @p prec_op_finer consider the case where you have a matrix @var A and you want to provide an 'inverse' using AMG.
   * A possible strategy consist in using the linear operator associated to AMG and a further strategy is to invert @var A using AMG.
   * In detail, @p prec_op should be
   * \code{.cpp}
   ...
   * \endcode
   *
   * In the @var .prm file it is possible to specify the maximum number of iterations allowed for the solver in the case we are using @p prec_op or @p prec_op_finer.
   *
   * To enable the finer preconditioner it is sufficient to set "Enable finer preconditioner" equals to @var true.
   */
